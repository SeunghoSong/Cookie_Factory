...
* **📍 [Iteration 9.0] — 원근 왜곡 대응 원형도 컷오프 정밀 재조정**: 스마트폰 카메라 각도에 따른 찌그러짐을 반영하여 정상(OK) 판정 컷오프를 $c=0.82$에서 $c=0.75$로 현실적으로 상향 재조정하여 정밀도를 극대화.
* **📍 [Iteration 10.0] — 반쪽 쿠키(HALF) 검출을 위한 방어막 튜닝**: 절단된 쿠키가 먼지로 오인되어 버려지는 현상을 막기 위해 C2 최소 면적 하한을 1000에서 500으로, 종횡비를 0.25~4.0으로 대폭 완화하여 초소형 파편 객체도 100% 잡아내도록 개선.
* **💡 [Iteration 11.0] · Border Rejection (경계선 오탐지) 방어막 도입**: 과자가 화면 테두리에 걸쳐서 진입할 때 짤린 상태로 인식되어 '불량'으로 오탐지되는 현상을 원천 차단하기 위해, C2 엔진에 마진(Margin) 10픽셀을 두어 화면 안으로 완전히 진입한 객체만 분석하도록 최적화.
* **✨ [Iteration 12.0] · End-to-End 실시간 스트리밍 랙 제로(Zero-Lag) 최적화 및 팀원 UI 통합**: 모바일-서버 간 프레임 전송 주기를 250ms(4FPS)로 최적화하고, C1 게이트웨이에 'End-to-End 래치(Latch)' 알고리즘을 도입하여 5초 이상 밀리던 백프레셔(Backpressure) 랙을 완벽하게 제거. 또한, 팀원분이 제작한 고도화된 프론트엔드 UI(dashboard.html, mobile.html) 디자인을 100% 보존하면서 C1 백엔드 통신 스펙을 UI 요구사항에 맞게 맞춤형으로 패치하여 완벽한 통합 버전을 완성함.
