...
| C2 Preprocessor | 프레임에서 HSV 색상 마스킹(황토색)으로 쿠키 후보를 검출하고, 3개 컨베이어 라인(`LINE 01~03`)별 bbox·컨투어를 뽑아 C3로 전달 | C1이 HTTP POST(`/preprocess`)로 자동 호출, 별도 조작 불필요. 헬스체크: `http://localhost:5001/` | (작성 예정) |
| C3 Inference Core | 후보 컨투어의 원형도(`c = r_min/r_max`)를 계산해 `c < 0.71`이면 NG 판정, 결손 정도에 따라 `HALF/WEDGE/CUT/BITE`로 결함 유형 분류 | C2가 HTTP POST(`/infer`)로 자동 호출, 별도 조작 불필요. 헬스체크: `http://localhost:5002/` | (작성 예정) |
